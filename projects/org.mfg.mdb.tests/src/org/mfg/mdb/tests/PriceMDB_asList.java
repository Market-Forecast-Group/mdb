...
public class PriceMDB_asList {
	...
	public void test_with_seq_positions() throws IOException {
		...
		try (RandomCursor c = mdb.randomCursor()) {
			MDBList<Record> mdbList = mdb.list(c);
			for (int i = 0; i < list.size(); i++) {
				Record r1 = list.get(i);
				Record r2 = mdbList.get(i);
	...
	public void test_with_random_positions() throws IOException {
		...
		try (RandomCursor c = mdb.randomCursor()) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				Record r1 = list.get(pos);
				Record r2 = mdbList.get(pos);
				assertArrayEquals("Record " + pos, r1.toArray(), r2.toArray());
			}
		}
	}
}
