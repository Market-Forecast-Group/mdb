...
/**
 * <p>
 * This class provides the API to manipulate Person files. 
 * Here you will find the methods to modify and query the Person files. 
 * </p>
 * <p>
 ...
 * <h3>PersonMDB API</h3>
 * <p>
 * Now let's see the operations you can perform using this class on Person files:
 * </p>
 *
 ...
 * PersonMDB mdb = session.connectTo_PersonMDB("person.mdb");
 * 
 * // request the appender.
 * PersonMDB.Appender app = mdb.appender(); 
 *
 ...
 * Person.Cursor cursor = mdb.cursor(start, stop);
 *
 ...
 * PersonMDB.RandomCursor cursor = mdb.randomCursor();
 *
 ...
 * </p>
 * <ul>
 * <li>{@link PersonMDB#replace_age(long, int)}: To replace the age value.</li>
 * <li>{@link PersonMDB#replace_phone(long, int)}: To replace the phone value.</li>
 * <li>{@link PersonMDB#replace_name(long, String)}: To replace the name value.</li>
 * </ul>
 *
 * <h3>List API</h3>
 * TODO: Documentation is comming
 *
 * @see JUnitTestsMDBSession#connectTo_PersonMDB(String)
 */

public final class PersonMDB
/* BEGIN MDB EXTENDS */
extends MDB<PersonMDB.Record>
/* END MDB EXTENDS */
...
/* BEGIN USER MDB */
	...
	/* END USER MDB */
	/**
	 * Person's meta-data: column names.
	 */
	...
	/**
	 * Person's meta-data: column Java types.
	 */
	...
	/**
	 * Person's meta-data: column Java types size (in bytes).
	 */
	...
	/**
	 * Person's meta-data: virtual column flags.
	 */
	...
	/**
	 * Person's meta-data: column byte-offset.
	 */
	...
	/**
	 * Person's meta-data: size of the record, in bytes.
	 */
	public static final int RECORD_SIZE = 20;
	
	/**
	* age's meta-data: index in a record.
	*/	
	public static final int COLUMN_AGE = 0;
	/**
	* phone's meta-data: index in a record.
	*/	
	public static final int COLUMN_PHONE = 1;
	/**
	* name's meta-data: index in a record.
	*/	
	public static final int COLUMN_NAME = 2;

	/**
	 * Person's meta-data: UUID used in schemas.
	 */
	public static final String TABLE_ID = "30ff641d-1bcb-4d52-b53f-a6c885206c58";
	
	/**
	 * Person's meta-data: signature used to check schema changes.
	 */ 
	public static final String TABLE_SIGNATURE = "8263ce11-c736-4227-9324-050f07a20082 INTEGER; 0466fd5d-ce61-41bb-8272-fdac107eaca3 INTEGER; 3eb2417b-1d27-4d76-8fce-cd4167c4f61e STRING; ";


	private Appender _appender;
	private ByteBuffer _replaceBuffer_age;
	private ByteBuffer _replaceBuffer_phone;
	int _rbufSize;
	 ...
	 */
	public PersonMDB(JUnitTestsMDBSession session, File file, File arrayFile, int bufferSize, SessionMode mode) throws IOException {
		...
		if (!_memory) {		
			_replaceBuffer_age = ByteBuffer.allocate(4);
			_replaceBuffer_phone = ByteBuffer.allocate(4);
		}
	}	
...
/* END RECORD EXTENDS */	{
		...
		* <h2>Definition</h2>
		* <table border=1>
		* <caption>Definition of age</caption>
		* <tr><td>Column</td><td>age</td></tr>
		...
		*/
		public int age; /* 0 */
		/**
		* Represents the phone column.
		* <h2>Definition</h2>
		* <table border=1>
		* <caption>Definition of phone</caption>
		* <tr><td>Column</td><td>phone</td></tr>
		* <tr><td>Type</td><td>INTEGER</td></tr>
		* <tr><td>Order</td><td>NONE</td></tr>
		* <tr><td>Virtual</td><td>No</td></tr>
		* </table>
		...
		* <h2>Definition</h2>
		* <table border=1>
		* <caption>Definition of name</caption>
		* <tr><td>Column</td><td>name</td></tr>
...
/**
	...
	public final class Appender implements IAppender<Record> {
		...
		*/
		Appender() throws IOException {
			_mdb = PersonMDB.this;
			if (!_memory) {
				_buf = ByteBuffer.allocate(_bufferSize * 20);
				reconnectFile();
			}
		}
		...
		public void append() throws IOException {
			if (_basic) {
				try {
					...
					_buf.putInt(this.age);
					_buf.putInt(this.phone);
					{
						byte[] __name = this.name == null? null : this.name.getBytes();
						int len = __name == null? 0 : __name.length;
						_buf.putLong(_arrayChannel.position());
						_buf.putInt(len * 1);
						if (len > 0) {
							ByteBuffer arrayBuf = ByteBuffer.wrap(new byte[len * 1]);
							for (int i = 0; i < len; i++) {
								arrayBuf.put(__name[i]); 
							}
							arrayBuf.rewind();
		...
		*/
		public void flush() throws IOException {	
			...
			for(int j = 0; j < _rbufSize; j++) {						
				...
				_buf.putInt(r.phone);
				{
					byte[] __name = r.name == null? null : r.name.getBytes();
					int len = __name == null? 0 : __name.length;
					_buf.putLong(_arrayChannel.position());
					_buf.putInt(len * 1);
					if (len > 0) {
		...
		*/
		Cursor(RandomAccessFile raf, FileChannel channel, long start, long stop, int bufferSize) throws IOException {
			...
			if (!_memory) {
				_raf = raf;
				_channel = channel;
				_channel.position(start * 20);
				_buffer = ByteBuffer.allocate(bufferSize * 20);
		...
		*/
		...
		public synchronized boolean next() throws IOException {
			...
			if (!_memory) {
				...
				this.age = _buffer.getInt();
				this.phone = _buffer.getInt();
				{
					...
					arrayBuf.rewind();
					byte[] __name = new byte[byteLen / 1]; 
					for(int i = 0; i < __name.length; i++) {
						__name[i] = arrayBuf.get(); 
					}
					this.name = new String(__name); 
				}
				_row ++;
			}
			return true;
		...
		public final class RandomCursor implements IRandomCursor<Record> {
			...
			RandomCursor() throws IOException {
				...
				if (!_memory) {
					...
					_channel = _raf.getChannel();
					_buffer = ByteBuffer.allocate(20);
					_arrayRaf = new RandomAccessFile(getArrayFile(),"rw");
					_arrayChannel = _arrayRaf.getChannel();
				}
				_session.cursorCreated(this);
			...
			public synchronized void seek(long position) throws IOException {
				...
				_row = position;
				_buffer.rewind();
				_channel.read(_buffer, position * 20);
				_buffer.rewind();
				this.age = _buffer.getInt();
				this.phone = _buffer.getInt();
				{
	...
	*/
	 ...
	 * but only the column "age" is updated. If you like to 
	 * update many fields of the record then use {@link #replace(long , int, int)},
	 ...
	 * @param value The new value to set to the column "age".
	 *
	 * @throws IOException If there is any I/O error.
	 */
	public void replace_age(long index, int value) throws IOException {
		...
		try {
			...
			_replaceBuffer_age.rewind();
			_replaceBuffer_age.putInt(value);
			_replaceBuffer_age.rewind();
			appender();
			_appender._channel.write(_replaceBuffer_age, index * 20 + 0);
		} finally {
	...
	/**
	 * Update the record at the given <code>index</code>, 
	 * but only the column "phone" is updated. If you like to 
	 * update many fields of the record then use {@link #replace(long , int, int)},
	 * but if you want to update just one column, use this method because it is faster.
	 * 
	 * @param index The index of the record to update.
	 * @param value The new value to set to the column "phone".
	 *
	 * @throws IOException If there is any I/O error.
	 */
	public void replace_phone(long index, int value) throws IOException {
		if (!_basic) _writeLock.lock();
		try {
			if (!_basic) {
				if (index >= _rbufPos) {
					int pos = (int) (index - _rbufPos);
					if (pos < _rbufSize) {
						_rbuf[pos].phone = value;
					}
					return;
				} 				
			}
		
			_replaceBuffer_phone.rewind();
			_replaceBuffer_phone.putInt(value);
			_replaceBuffer_phone.rewind();
			appender();
			_appender._channel.write(_replaceBuffer_phone, index * 20 + 4);
		} finally {
			if (!_basic) _writeLock.unlock();
		}		
	}
	...
	public long fsize() throws IOException {		
		return _memory? 0 : _file.length() / 20;
	}
