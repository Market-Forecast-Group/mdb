...
/**
 * <p>
 * This class provides the API to manipulate Table4 files. 
 * Here you will find the methods to modify and query the Table4 files. 
 * </p>
 * <p>
 ...
 * <h3>Table4MDB API</h3>
 * <p>
 * Now let's see the operations you can perform using this class on Table4 files:
 * </p>
 *
 ...
 * Table4MDB mdb = session.connectTo_Table4MDB("table4.mdb");
 * 
 * // request the appender.
 * Table4MDB.Appender app = mdb.appender(); 
 *
 ...
 * Table4.Cursor cursor = mdb.cursor(start, stop);
 *
 ...
 * Table4MDB.RandomCursor cursor = mdb.randomCursor();
 *
 ...
 * </p>
 * <ul>
 * <li>{@link Table4MDB#replace_num(long, byte)}: To replace the num value.</li>
 * <li>{@link Table4MDB#replace_flag(long, boolean)}: To replace the flag value.</li>
 * </ul>
 *
 * <h3>List API</h3>
 * TODO: Documentation is comming
 *
 * @see JUnitTestsMDBSession#connectTo_Table4MDB(String)
 */

public final class Table4MDB
/* BEGIN MDB EXTENDS */
extends MDB<Table4MDB.Record>
/* END MDB EXTENDS */
{
...
/* BEGIN USER MDB */
	...
	/* END USER MDB */
	/**
	 * Table4's meta-data: column names.
	 */
	...
	/**
	 * Table4's meta-data: column Java types.
	 */
	...
	/**
	 * Table4's meta-data: column Java types size (in bytes).
	 */
	...
	/**
	 * Table4's meta-data: virtual column flags.
	 */
	...
	/**
	 * Table4's meta-data: column byte-offset.
	 */
	...
	/**
	 * Table4's meta-data: size of the record, in bytes.
	 */
	public static final int RECORD_SIZE = 2;
	
	/**
	* num's meta-data: index in a record.
	*/	
	public static final int COLUMN_NUM = 0;
	/**
	* flag's meta-data: index in a record.
	*/	
	public static final int COLUMN_FLAG = 1;

	/**
	 * Table4's meta-data: UUID used in schemas.
	 */
	public static final String TABLE_ID = "198733bc-715a-45f7-aeac-d06a40a31fda";
	
	/**
	 * Table4's meta-data: signature used to check schema changes.
	 */ 
	public static final String TABLE_SIGNATURE = "25237234-4911-47ec-a63f-d28f6dc42125 BYTE; 13010d24-d152-42cd-b2c8-42d915725a76 BOOLEAN; ";


	private Appender _appender;
	private ByteBuffer _replaceBuffer; 
	private ByteBuffer _replaceBuffer_num;
	private ByteBuffer _replaceBuffer_flag;
	int _rbufSize;
	 ...
	 */
	public Table4MDB(JUnitTestsMDBSession session, File file, int bufferSize, SessionMode mode) throws IOException {
		...
		if (!_memory) {		
			_replaceBuffer = ByteBuffer.allocate(2);		
			_replaceBuffer_num = ByteBuffer.allocate(1);
			_replaceBuffer_flag = ByteBuffer.allocate(1);
		}
	}	

	/**
	* Table4 record structure.
	*/
...
/* END RECORD EXTENDS */	{
		/**
		* Represents the num column.
		* <h2>Definition</h2>
		* <table border=1>
		* <caption>Definition of num</caption>
		* <tr><td>Column</td><td>num</td></tr>
		...
		*/
		public byte num; /* 0 */
		/**
		* Represents the flag column.
		* <h2>Definition</h2>
		* <table border=1>
		* <caption>Definition of flag</caption>
		* <tr><td>Column</td><td>flag</td></tr>
		* <tr><td>Type</td><td>BOOLEAN</td></tr>
		* <tr><td>Order</td><td>NONE</td></tr>
		* <tr><td>Virtual</td><td>No</td></tr>
		* </table>
		*/
		public boolean flag; /* 1 */
...
/**
	...
	public final class Appender implements IAppender<Record> {
		...
		*/
		Appender() throws IOException {
			_mdb = Table4MDB.this;
			if (!_memory) {
				_buf = ByteBuffer.allocate(_bufferSize * 2);
				reconnectFile();
			}
		}
		...
		*/
		public void flush() throws IOException {	
			...
			for(int j = 0; j < _rbufSize; j++) {						
				Record r = _rbuf[j];
				_buf.put(r.num);
				_buf.put((byte) (r.flag? 1 : 0));
			}
			_buf.rewind();
			_buf.limit(_rbufSize * 2);
			_channel.write(_buf);
			_buf.limit(_buf.capacity());
			_buf.rewind();
		...
		*/
		Cursor(RandomAccessFile raf, FileChannel channel, long start, long stop, int bufferSize) throws IOException {
			...
			if (!_memory) {
				_raf = raf;
				_channel = channel;
				_channel.position(start * 2);
				_buffer = ByteBuffer.allocate(bufferSize * 2);
		...
		*/
		...
		public synchronized boolean next() throws IOException {
			...
			if (!_memory) {
				...
				this.num = _buffer.get();
				this.flag = _buffer.get() == 0? false : true;
				_row ++;
			}
			return true;
		}
		...
		*/
		public final class RandomCursor implements IRandomCursor<Record> {
			...
			public synchronized void seek(long position) throws IOException {
				...
				_row = position;
				_buffer.rewind();
				_channel.read(_buffer, position * 2);
				_buffer.rewind();
				this.num = _buffer.get();
				this.flag = _buffer.get() == 0? false : true;
			}
	...
	*/
	...
	public static void writeBuffer(Cursor obj, ByteBuffer buffer) {
		buffer.put(obj.num); 
		buffer.put(obj.flag ? (byte) 1 : (byte) 0); 
	}
	 ...
	 * @param val_num The value for column num.
	 * @param val_flag The value for column flag.
	* @throws IOException If there is any I/O error.
	 */
	public void replace(long index 
		...
		try {
			...
			_replaceBuffer.put(val_num);
			_replaceBuffer.put((byte) (val_flag ? 1 : 0));
			_replaceBuffer.rewind();
			appender();
			_appender._channel.write(_replaceBuffer, index * 2);
		} finally {
	 ...
	 * Also you can use {@link #replace(long , byte, boolean)}.
	 *
	 * @param index The index to update.
	 * @param record Contains the data to set.
	 * @see #replace(long , byte, boolean)
	 * @throws IOException If there is any I/O error.
	 */
	...
	/**
	 * Update the record at the given <code>index</code>, 
	 * but only the column "num" is updated. If you like to 
	 * update many fields of the record then use {@link #replace(long , byte, boolean)},
	 ...
	 * @param value The new value to set to the column "num".
	 *
	 * @throws IOException If there is any I/O error.
	 */
	public void replace_num(long index, byte value) throws IOException {
		if (!_basic) _writeLock.lock();
		try {
			if (!_basic) {
				if (index >= _rbufPos) {
			...
			_replaceBuffer_num.rewind();
			_replaceBuffer_num.put(value);
			_replaceBuffer_num.rewind();
			appender();
			_appender._channel.write(_replaceBuffer_num, index * 2 + 0);
		} finally {
			if (!_basic) _writeLock.unlock();
		}		
	}
	/**
	 * Update the record at the given <code>index</code>, 
	 * but only the column "flag" is updated. If you like to 
	 * update many fields of the record then use {@link #replace(long , byte, boolean)},
	 * but if you want to update just one column, use this method because it is faster.
	 * 
	 * @param index The index of the record to update.
	 * @param value The new value to set to the column "flag".
	 *
	 * @throws IOException If there is any I/O error.
	 */
	public void replace_flag(long index, boolean value) throws IOException {
		if (!_basic) _writeLock.lock();
		try {
			if (!_basic) {
				if (index >= _rbufPos) {
					int pos = (int) (index - _rbufPos);
					if (pos < _rbufSize) {
						_rbuf[pos].flag = value;
					}
					return;
				} 				
			}
		
			_replaceBuffer_flag.rewind();
			_replaceBuffer_flag.put((byte) (value ? 1 : 0));
			_replaceBuffer_flag.rewind();
			appender();
			_appender._channel.write(_replaceBuffer_flag, index * 2 + 1);
		} finally {
			if (!_basic) _writeLock.unlock();
		}		
	}
	...
	public long fsize() throws IOException {		
		return _memory? 0 : _file.length() / 2;
	}
